...
/**
 * Klapt het blok open, zoals een gebruiker dat doet: met een tik op de regel.
 *
 ...
 * wanneer een echte browser ze niet toont. Zonder deze stap zou een test groen staan op
 * knoppen die niemand kan zien.
 */
...
describe('OverzichtKaartkeuze (ronde 90)', () => {
  ...
  it('staat standaard DICHT, en klapt open met één tik op de regel', async () => {
    // ⚠ De kern van deze ronde. Open beslaat de chiprij op een telefoon van 360 px
    // 269 px — vier rijen chips, gemeten in Chromium — en dat op de pagina waar je
    // landt. Zou dit blok ooit standaard opengaan, dan voegt de knop die het scherm
    // moet inkorten er zelf een derde scherm aan toe.
  ...
  it('geeft elke kaart die er kán staan een chip', async () => {
    const user = userEvent.setup()
    toon()
    await klapOpen(user)
  ...
  it('biedt GEEN chip voor het maandblok of de zijkolom', async () => {
  ...
  it('staat standaard alles aan', async () => {
    const user = userEvent.setup()
    toon()
    await klapOpen(user)
  ...
  it('hangt de belofte aan élke chip', async () => {
  ...
  it('laat een kaart zonder gegevens weg, en NOEMT ZE BIJ NAAM', async () => {
  ...
  it('zwijgt over weggelaten kaarten zodra ze er allemaal zijn', async () => {
    const user = userEvent.setup()
    toon()
    await klapOpen(user)
  ...
  it('trekt zich niets aan van een bewaarde voorkeur die geen lijst is', async () => {
  ...
  it('toont alles wanneer de bewaarde voorkeur kapot is', async () => {
